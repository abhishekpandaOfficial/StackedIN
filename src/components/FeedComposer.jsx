...
import AIProviderControls, { AI_PROVIDERS, AIProviderIcon, AIResponseAttribution } from "./AIProviderControls.jsx";
import SocialAccountManager, { ProviderIcon, SOCIAL_PROVIDERS } from "./SocialAccountManager.jsx";
import { getAIProviderConfiguration } from "../services/aiProviderSession.js";
...
export default function FeedComposer({ session, tenantContext, onPublished, openArticle, onToast }) {
  ...
  const [aiProvider, setAiProvider] = useState("sarvam");
  const [aiModel, setAiModel] = useState("sarvam-105b");
  ...
  const runAIAssist = async () => {
    ...
    try {
      ...
      const personalConfiguration = getAIProviderConfiguration(aiProvider);
      const response = await fetch("/api/ai-writing", {
        method: "POST",
        headers: { "content-type": "application/json", authorization: `Bearer ${data.session?.access_token || ""}` },
        body: JSON.stringify({ provider: aiProvider, model: aiModel, apiKey: personalConfiguration?.apiKey || "", prompt: aiPrompt, currentText: body }),
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.error || "AI writing assistance is unavailable.");
      setBody(payload.text || body);
      setLastAIResponse({ provider: payload.provider || aiProvider, model: payload.model || aiModel });
      setAiAssistOpen(false);
      onToast?.(`${AI_PROVIDERS[payload.provider || aiProvider]?.label || aiProvider} · ${payload.model || aiModel} draft added. Review it in your own voice before publishing.`);
  ...
  return <>
    ...
    {open && <div className="post-composer-backdrop" onMouseDown={event => event.target === event.currentTarget && !busy && setOpen(false)}>
      <section className="post-composer-dialog" role="dialog" aria-modal="true" aria-labelledby="post-composer-title">
        ...
        {lastAIResponse && <AIResponseAttribution response={lastAIResponse} />}
        {aiAssistOpen && <section className="composer-ai-assist"><header><div><Sparkles size={15} /><span><strong>AI writing assistant</strong><small>Sarvam is included. Personal provider keys stay in memory for this session only.</small></span></div><AIProviderIcon provider={aiProvider} size={18} /></header><AIProviderControls provider={aiProvider} model={aiModel} onProviderChange={setAiProvider} onModelChange={setAiModel} /><textarea value={aiPrompt} onChange={event => setAiPrompt(event.target.value)} maxLength={2000} placeholder="Describe the post, tone, audience, facts, and outcome. The assistant will draft; you remain the editor." /><button disabled={!aiPrompt.trim() || busy === "ai" || (aiProvider !== "sarvam" && !getAIProviderConfiguration(aiProvider))} onClick={runAIAssist}>{busy === "ai" ? <LoaderCircle className="spin" size={14} /> : <Sparkles size={14} />}Generate with {AI_PROVIDERS[aiProvider]?.label}</button></section>}
