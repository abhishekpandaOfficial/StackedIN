...
const routePathFor = (route = "home") => `${appBasePath()}${route === "home" ? "/" : `/${String(route).replace(/^\/+|\/+$/g, "")}`}`;
const legacyRouteFromHash = () => {
  const hash = window.location.hash.replace(/^#\/?/, "");
  if (!hash || hash.includes("access_token=") || hash.includes("refresh_token=")) return "";
  if (/^article-[0-9a-f-]+$/i.test(hash)) return `article/${hash.slice(8)}`;
  if (/^profile-[0-9a-f-]+$/i.test(hash)) return `profile/${hash.slice(8)}`;
  if (/^(feed(?:\/(?:following|subscribed|saved))?|network|search|profile(?:\/[^/?#]+)?|inbox|write|studio|login)$/.test(hash)) return hash;
  return "";
};
...
const profileUrlFor = (username) => new URL(routePathFor(`profile/${encodeURIComponent(username || "")}`), window.location.origin).href;
const slugOf = (url) => {
  try {
...
function MarketingLanding({ openStudio }) {
  ...
  return <div className="stackedin-site">
    <header className="marketing-nav">
      <button className="marketing-brand" onClick={() => scrollToSection("top")} aria-label="StackedIN home"><img src={`${base}stackedin-wordmark.webp`} alt="StackedIN" /></button>
      <nav aria-label="Marketing navigation"><button onClick={() => scrollToSection("why")}>Why StackedIN</button><button onClick={() => scrollToSection("experience")}>Experience</button><button onClick={() => scrollToSection("ecosystem")}>Ecosystem</button><button onClick={() => scrollToSection("how")}>How it works</button></nav>
    ...
    <main id="top">
      <section className="marketing-hero">
        ...
        <div className="hero-copy">
          ...
          <div className="hero-actions"><button className="hero-primary" onClick={() => scrollToSection("why")}>Explore StackedIN <ArrowRight size={17} /></button><button className="hero-secondary" onClick={openStudio}>View publishing studio</button></div>
        ...
        <div className="hero-stage" aria-label="Animated StackedIN publishing preview">
          <div className="hero-sequence" aria-hidden="true"><span>STK / 01</span><i /><b>KNOWLEDGE<br />IN MOTION</b></div>
          <div className="hero-kinetic-type" aria-hidden="true"><span>BUILD</span><span>WRITE</span><span>CONNECT</span></div>
      ...
      <section className="footer-community" aria-label="StackedIN professional community"><div className="footer-community-copy"><span>Meet through what matters</span><h2>People move.<br />Ideas travel further.</h2><p>A professional network where conversations begin with useful work—not cold outreach.</p><button onClick={openStudio}>Start your StackedIN journey <ArrowRight size={15} /></button></div><div className="footer-community-stage"><i className="talk-bubble bubble-one"><span /><span /><span /></i><i className="talk-bubble bubble-two"><MessageCircle size={13} /></i><img className="footer-community-people" loading="lazy" src={`${base}stackedin-community-footer.webp`} alt="A diverse group of professionals walking, collaborating, and talking" /><div className="walking-line" /></div></section>
    </main>
  ...
  const saveProfile = async () => {
    ...
    try {
      if (draft.username !== bundle.profile.username) {
        ...
        window.history.replaceState(null, "", routePathFor(`profile/${encodeURIComponent(claimedUsername)}`));
      }
...
function MessagingExperience({ session, initialConversationId, openFeed, openProfile, openWrite, openStudio, openArticle }) {
  const [context, setContext] = useState(null);
  ...
  const openNotification = async (notification) => {
    ...
    if (notification.entity_type === "ARTICLE" && notification.entity_id) openArticle(notification.entity_id);
    else await load();
  };
...
function FeedExperience({ session, feedMode = "all", openFeedMode, openStudio, openNetwork, openSearch, openWrite, openProfile, openInbox, signOut }) {
  ...
  return <div className={`feed-page ${feedPreferences.compact ? "feed-compact" : ""} ${feedPreferences.reduceMotion ? "feed-reduce-motion" : ""}`}>
    <header className="feed-topbar"><button className="feed-logo" onClick={() => openFeedMode("all")}><img src={`${import.meta.env.BASE_URL}stackedin-icon.webp`} alt="StackedIN" /></button><label><Search size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => {
...
function NetworkExperience({ session, openFeed, openSearch, openProfile, openStudio, signOut }) {
  ...
  return <div className="feed-page network-page">
    <header className="feed-topbar"><button className="feed-logo" onClick={openFeed}><img src={`${import.meta.env.BASE_URL}stackedin-icon.webp`} alt="StackedIN" /></button><label><Search size={17} /><input value={networkSearch} onChange={(event) => setNetworkSearch(event.target.value)} onKeyDown={(event) => {
...
function App() {
  const [route, setRoute] = useState(() => {
    ...
    if (legacyRoute) window.history.replaceState(null, "", routePathFor(legacyRoute));
    return legacyRoute || routeFromLocation();
  });
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, nextSession) => {
      ...
      if (nextSession && ["home", "login"].includes(currentRoute)) {
        window.history.replaceState(null, "", routePathFor("feed"));
        setRoute("feed");
      }
      if (event === "SIGNED_OUT") {
        window.history.replaceState(null, "", routePathFor("home"));
        setRoute("home");
      }
    });
  ...
  const navigate = (next) => {
    window.history.pushState(null, "", routePathFor(next));
    setRoute(next);
    window.scrollTo({ top: 0, behavior: "instant" });
  };
  useEffect(() => {
    const routeKey = route.startsWith("profile/") ? "profile" : route.startsWith("article/") ? "article" : route.startsWith("feed/") ? "feed" : route;
    const metadata = {
      home: ["StackedIN — The Professional Knowledge Network", "Publish deeply, connect through expertise, and build a professional body of work."],
      login: ["Sign in to StackedIN", "Access your StackedIN professional network and publishing workspace."],
      feed: ["Professional Feed | StackedIN", "Discover professional posts, articles, discussions, and people in your knowledge network."],
      network: ["My Network | StackedIN", "Manage professional connections, follows, subscriptions, and recommendations."],
      search: ["Search Professionals | StackedIN", "Find professionals, expertise, articles, and shared interests on StackedIN."],
      profile: ["Professional Profile | StackedIN", "Explore a StackedIN professional journey, experience, skills, and published work."],
      inbox: ["Inbox | StackedIN", "View private messages, connection requests, and notifications."],
      write: ["Write | StackedIN", "Create a post or article for the StackedIN professional network."],
      studio: ["XStudio CMS | StackedIN", "Write, organize, schedule, and distribute professional content from one tenant-private workspace."],
      article: ["Article | StackedIN", "Read and discuss professional knowledge published on StackedIN."]
    }[routeKey] || ["StackedIN", "The professional knowledge network."];
    document.title = metadata[0];
    document.querySelector('meta[name="description"]')?.setAttribute("content", metadata[1]);
    for (const [selector, value] of [['meta[property="og:title"]', metadata[0]], ['meta[property="og:description"]', metadata[1]], ['meta[name="twitter:title"]', metadata[0]], ['meta[name="twitter:description"]', metadata[1]]]) document.querySelector(selector)?.setAttribute("content", value);
    let canonical = document.querySelector('link[rel="canonical"]');
    if (!canonical) { canonical = document.createElement("link"); canonical.rel = "canonical"; document.head.appendChild(canonical); }
    canonical.href = new URL(routePathFor(route), window.location.origin).href;
  }, [route]);
  ...
  const protectedRoute = ["feed", "network", "search", "profile", "inbox", "write", "studio"].includes(route) || route.startsWith("feed/") || route.startsWith("article/") || route.startsWith("profile/");
  ...
  if ((route === "profile" || route.startsWith("profile/")) && session) return <ProfileExperience session={session} targetProfileRef={route.startsWith("profile/") ? decodeURIComponent(route.slice(8)) : session.user.id} openFeed={() => navigate("feed")} openWrite={() => navigate("write")} openInbox={openInbox} openStudio={() => navigate("studio")} signOut={signOut} />;
  if (route === "inbox" && session) return <MessagingExperience session={session} initialConversationId={sessionStorage.getItem("stackedin-inbox-conversation") || ""} openFeed={() => navigate("feed")} openProfile={openProfile} openWrite={() => navigate("write")} openStudio={() => navigate("studio")} openArticle={(id) => navigate(`article/${id}`)} />;
