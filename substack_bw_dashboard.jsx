...
import {
  ...
  Award, Building2, Camera, Check, ChevronRight, Clock3, ExternalLink, Eye, EyeOff, FileText, Filter, Flag, GraduationCap, Grid2X2, Heart, Home, KeyRound, Layers3, Link2,
  ...
  BellRing, Plus, Trash2, UserMinus, UserRound, Users, Workflow, X, Zap,
} from "lucide-react";
...
function ProfileRecordEditor({ type, record, onSave, onCancel, busy }) {
  ...
  const submit = event => { event.preventDefault(); const payload = { ...draft }; if ("skills" in payload) payload.skills = String(payload.skills || "").split(",").map(item => item.trim()).filter(Boolean); if (payload.currently_working) payload.end_date = null; for (const key of ["end_date","start_date","issued_on","project_url","repository_url","image_url","credential_url"]) if (payload[key] === "") payload[key] = null; onSave(config.table, payload); };
...
function ProfileExperience({ session, targetProfileId, openFeed, openWrite, openInbox, signOut }) {
  ...
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState("");
  const avatarInput = useRef(null); const bannerInput = useRef(null);
  const isOwner = targetProfileId === session.user.id;
  const load = useCallback(async () => {
    setBusy(true);
    try {
      const nextContext = context || await loadTenantContext(session.user.id); setContext(nextContext);
      const nextBundle = await profileHub.loadProfile(targetProfileId, nextContext.tenant.id, session.user.id); setBundle(nextBundle); setDraft(nextBundle.profile);
    } catch (error) { setMessage(error.message || "Profile could not be loaded."); }
    finally { setBusy(false); }
  }, [context, session.user.id, targetProfileId]);
  useEffect(() => { load(); }, [load]);
  useEffect(() => { const channel = profileHub.subscribe(targetProfileId, load); return () => { void supabase.removeChannel(channel); }; }, [targetProfileId, load]);
  const saveProfile = async () => {
    ...
    try {
      ...
      changes.years_experience = changes.years_experience === "" ? null : Number(changes.years_experience || 0); for (const field of ["featured_skills","featured_badges"]) if (!Array.isArray(changes[field])) changes[field] = String(changes[field] || "").split(",").map(item => item.trim()).filter(Boolean);
      await profileHub.updateProfile(session.user.id, changes); setEditingProfile(false); setMessage("Professional profile saved."); await load();
    } catch (error) { setMessage(error.message || "Profile could not be saved."); }
    finally { setBusy(false); }
  };
  const uploadImage = async (event, kind) => {
    const file = event.target.files?.[0]; if (!file) return; setBusy(true);
    try { const url = await profileHub.uploadProfileImage(session.user.id, file, kind); await profileHub.updateProfile(session.user.id, { [kind === "avatar" ? "avatar_url" : "banner_url"]: url }); await load(); }
    catch (error) { setMessage(error.message || "Image could not be uploaded."); } finally { setBusy(false); event.target.value = ""; }
  };
  const saveRecord = async (table, record) => { setBusy(true); try { await profileHub.saveRecord(table, context.tenant.id, session.user.id, record); setRecordEditor(null); await load(); } catch (error) { setMessage(error.message); } finally { setBusy(false); } };
  const deleteRecord = async (type, id) => { setBusy(true); try { await profileHub.deleteRecord(PROFILE_RECORD_CONFIG[type].table, session.user.id, id); await load(); } catch (error) { setMessage(error.message); } finally { setBusy(false); } };
  const relationshipAction = async action => {
    if (!context || isOwner) return; setBusy(true);
    try {
      ...
      if (action === "message") { const conversationId = await profileHub.startConversation(context.tenant.id, targetProfileId); openInbox(conversationId); return; }
      await load();
  ...
  const baseFields = [["display_name","Display name"],["headline","Headline"],["current_job_title","Current role"],["current_company","Company"],["industry","Industry"],["location","Location"],["country","Country"],["years_experience","Years experience"],["website_url","Website"],["github_url","GitHub"],["gitlab_url","GitLab"],["linkedin_url","LinkedIn"],["medium_url","Medium"],["hashnode_url","Hashnode"]];
  return <div className="profile-page"><header className="feed-topbar"><button className="feed-logo" onClick={openFeed}><img src={`${import.meta.env.BASE_URL}stackedin-icon.webp`} alt="StackedIN" /></button><label><Search size={17} /><input value="Professional profile" readOnly /></label><nav><button onClick={openFeed}><Home size={18} /><span>Home</span></button><button onClick={() => openInbox()}><Bell size={18} /><span>Inbox</span></button>{isOwner && <button onClick={openWrite}><PenTool size={18} /><span>Write</span></button>}<button className="active feed-avatar"><b>{name.charAt(0).toUpperCase()}</b><span>{isOwner ? "Me" : "Profile"}</span></button></nav></header><main className="profile-journey-shell"><section className="profile-journey-hero"><div className="profile-journey-banner" style={profile.banner_url ? { backgroundImage:`url(${profile.banner_url})` } : undefined}>{isOwner && <div className="profile-media-actions"><button onClick={() => bannerInput.current?.click()}><Camera size={14} />Change banner</button>{profile.banner_url && <button onClick={() => { profileHub.updateProfile(session.user.id,{banner_url:null}).then(load); }}><Trash2 size={14} /></button>}<input ref={bannerInput} hidden type="file" accept="image/*" onChange={event => uploadImage(event,"banner")} /></div>}</div><div className="profile-journey-identity"><div className="profile-journey-avatar">{profile.avatar_url ? <img src={profile.avatar_url} alt="" /> : name.charAt(0).toUpperCase()}{isOwner && <button onClick={() => avatarInput.current?.click()}><Camera size={14} /></button>}<input ref={avatarInput} hidden type="file" accept="image/*" onChange={event => uploadImage(event,"avatar")} /></div><section><div><h1>{name}</h1><CheckCircle2 size={19} /></div><p>{profile.headline || [profile.current_job_title,profile.current_company].filter(Boolean).join(" · ") || "StackedIN professional"}</p><span><MapPin size={14} />{[profile.location,profile.country].filter(Boolean).join(", ") || "Location not specified"}<Users size={14} />{bundle.counts.followers} followers<Users size={14} />{bundle.counts.connections} connections</span><div className="profile-skill-chips">{(profile.featured_skills || []).slice(0,8).map(skill => <b key={skill}>{skill}</b>)}</div><div className="profile-badge-chips">{(profile.featured_badges || []).slice(0,5).map(badge => <b key={badge}><Award size={12} />{badge}</b>)}</div></section><aside>{isOwner ? <><button className="primary" onClick={() => setEditingProfile(true)}><PenTool size={15} />Edit profile</button><button onClick={openWrite}><Plus size={15} />Add post</button></> : <><button className="primary" disabled={busy || bundle.relationship.connectionStatus} onClick={() => relationshipAction("connect")}><Users size={15} />{bundle.relationship.connectionStatus === "PENDING" ? "Pending" : bundle.relationship.connectionStatus === "ACCEPTED" ? "Connected" : "Connect"}</button><button disabled={bundle.relationship.connectionStatus !== "ACCEPTED"} onClick={() => relationshipAction("message")}><MessageCircle size={15} />Message</button><button className={bundle.relationship.isFollowing ? "active" : ""} onClick={() => relationshipAction("follow")}><UserRound size={15} />{bundle.relationship.isFollowing ? "Following" : "Follow"}</button></>}</aside></div></section><nav className="profile-tabs">{tabs.map(([id,label]) => <button key={id} className={tab === id ? "active" : ""} onClick={() => setTab(id)}>{label}</button>)}</nav>{message && <div className="profile-journey-message">{message}</div>}{editingProfile && <section className="profile-base-editor"><header><div><span>Owner controls</span><h2>Edit professional identity</h2></div><button onClick={() => setEditingProfile(false)}><X size={16} /></button></header><div>{baseFields.map(([field,label]) => <label key={field}>{label}<input type={field === "years_experience" ? "number" : field.endsWith("_url") ? "url" : "text"} value={draft[field] || ""} onChange={event => setDraft(current => ({...current,[field]:event.target.value}))} /></label>)}<label className="wide">About<textarea value={draft.about || ""} onChange={event => setDraft(current => ({...current,about:event.target.value}))} /></label><label className="wide">Featured skills<input value={Array.isArray(draft.featured_skills) ? draft.featured_skills.join(", ") : draft.featured_skills || ""} onChange={event => setDraft(current => ({...current,featured_skills:event.target.value}))} placeholder="Azure, AI Architecture, .NET" /></label><label className="wide">Badges and recognitions<input value={Array.isArray(draft.featured_badges) ? draft.featured_badges.join(", ") : draft.featured_badges || ""} onChange={event => setDraft(current => ({...current,featured_badges:event.target.value}))} placeholder="Top Voice, Community Builder" /></label></div><footer><button onClick={() => setEditingProfile(false)}>Cancel</button><button onClick={saveProfile} disabled={busy}><Check size={15} />Save profile</button></footer></section>}{recordEditor && <ProfileRecordEditor type={recordEditor.type} record={recordEditor.record} onSave={saveRecord} onCancel={() => setRecordEditor(null)} busy={busy} />}<div className="profile-journey-grid"><section className="profile-journey-main">{tab === "about" && <><article className="journey-card"><header><div><span>About</span><h2>Professional story</h2></div></header><p>{profile.about || profile.bio || "This professional has not added an About section yet."}</p></article><article className="journey-card"><header><div><span>Education</span><h2>Learning journey</h2></div>{isOwner && <button onClick={() => setRecordEditor({type:"education",record:{}})}><Plus size={14} />Add</button>}</header>{bundle.education.map(item => <div className="journey-row" key={item.id}><GraduationCap size={20} /><section><h3>{item.institution}</h3><p>{[item.degree,item.field_of_study].filter(Boolean).join(" · ")}</p><span>{item.start_date || ""}{item.end_date ? ` — ${item.end_date}` : ""}</span></section>{isOwner && <div><button onClick={() => setRecordEditor({type:"education",record:item})}><PenTool size={13} /></button><button onClick={() => deleteRecord("education",item.id)}><Trash2 size={13} /></button></div>}</div>)}</article><article className="journey-card"><header><div><span>Recognition</span><h2>Achievements</h2></div>{isOwner && <button onClick={() => setRecordEditor({type:"achievement",record:{}})}><Plus size={14} />Add</button>}</header>{bundle.achievements.map(item => <div className="journey-row" key={item.id}><Award size={20} /><section><h3>{item.title}</h3><p>{item.issuer}</p><span>{item.description}</span></section>{isOwner && <div><button onClick={() => setRecordEditor({type:"achievement",record:item})}><PenTool size={13} /></button><button onClick={() => deleteRecord("achievement",item.id)}><Trash2 size={13} /></button></div>}</div>)}</article></>}{tab === "experience" && <article className="journey-card"><header><div><span>Career</span><h2>Experience</h2></div>{isOwner && <button onClick={() => setRecordEditor({type:"experience",record:{employment_type:"FULL_TIME"}})}><Plus size={14} />Add experience</button>}</header>{bundle.experiences.map(item => <div className="journey-row" key={item.id}><Building2 size={21} /><section><h3>{item.title}</h3><p>{item.company} · {String(item.employment_type || "").replaceAll("_"," ")}</p><span>{item.start_date} — {item.currently_working ? "Present" : item.end_date || "Present"}{item.location ? ` · ${item.location}` : ""}</span><p>{item.description}</p><div>{(item.skills || []).map(skill => <b key={skill}>{skill}</b>)}</div></section>{isOwner && <div><button onClick={() => setRecordEditor({type:"experience",record:item})}><PenTool size={13} /></button><button onClick={() => deleteRecord("experience",item.id)}><Trash2 size={13} /></button></div>}</div>)}</article>}{tab === "skills" && <article className="journey-card"><header><div><span>Expertise</span><h2>Skills and recognition</h2></div>{isOwner && <button onClick={() => setEditingProfile(true)}><PenTool size={14} />Edit</button>}</header><div className="journey-skills">{(profile.featured_skills || []).map(skill => <span key={skill}>{skill}</span>)}</div><div className="journey-badges">{(profile.featured_badges || []).map(badge => <span key={badge}><Award size={16} />{badge}</span>)}</div></article>}{tab === "portfolio" && <><article className="journey-card"><header><div><span>Selected work</span><h2>Portfolio</h2></div>{isOwner && <button onClick={() => setRecordEditor({type:"project",record:{}})}><Plus size={14} />Add project</button>}</header><div className="journey-projects">{bundle.projects.map(item => <article key={item.id}>{item.image_url && <img src={item.image_url} alt="" />}<h3>{item.title}</h3><p>{item.description}</p><div>{(item.skills || []).map(skill => <b key={skill}>{skill}</b>)}</div><footer>{item.project_url && <a href={item.project_url} target="_blank" rel="noreferrer">View project <ExternalLink size={12} /></a>}{item.repository_url && <a href={item.repository_url} target="_blank" rel="noreferrer">Repository <SiGithub size={12} /></a>}{isOwner && <><button onClick={() => setRecordEditor({type:"project",record:item})}><PenTool size={13} /></button><button onClick={() => deleteRecord("project",item.id)}><Trash2 size={13} /></button></>}</footer></article>)}</div></article><article className="journey-card"><header><div><span>Links</span><h2>Professional presence</h2></div>{isOwner && <button onClick={() => setRecordEditor({type:"link",record:{link_type:"OTHER"}})}><Plus size={14} />Add link</button>}</header><div className="journey-links">{socialLinks.map(([label,url,Icon]) => <a href={url} target="_blank" rel="noreferrer" key={`${label}-${url}`}><Icon size={18} /><span><strong>{label}</strong><small>{url}</small></span><ExternalLink size={13} /></a>)}</div></article></>}{tab === "activity" && <article className="journey-card"><header><div><span>Journey timeline</span><h2>Posts and professional activity</h2></div>{isOwner && <button onClick={openWrite}><Plus size={14} />Publish</button>}</header>{bundle.activities.map(item => <div className="journey-activity" key={item.id}><div><FileText size={16} /></div><section><span>{item.content_type} · {formatDate(item.published_at)}</span><h3>{item.title}</h3><p>{item.description}</p><footer>{item.reaction_count || 0} reactions · {item.comment_count || 0} comments · {item.restack_count || 0} restacks</footer></section></div>)}</article>}</section><aside className="profile-journey-side"><article><span>Profile completeness</span><div className="completion-ring" style={{"--completion":`${completion * 3.6}deg`}}><strong>{completion}%</strong><small>Complete</small></div></article><article><span>Professional signal</span><div className="signal-stats"><strong>{bundle.counts.publications}</strong><small>Publications</small><strong>{bundle.counts.followers}</strong><small>Followers</small><strong>{bundle.counts.connections}</strong><small>Connections</small></div></article>{socialLinks.length > 0 && <article><span>Connect elsewhere</span><div className="profile-side-links">{socialLinks.slice(0,6).map(([label,url,Icon]) => <a href={url} target="_blank" rel="noreferrer" key={`${label}-${url}`}><Icon size={16} />{label}<ExternalLink size={11} /></a>)}</div></article>}{isOwner && <article><button className="profile-signout" onClick={signOut}><LogOut size={14} />Sign out</button></article>}</aside></div></main></div>;
}

function InboxExperience({ session, initialConversationId, openFeed, openProfile }) {
  const [context, setContext] = useState(null);
  ...
  const load = useCallback(async () => {
    try {
      const nextContext = context || await loadTenantContext(session.user.id); setContext(nextContext);
      const [nextNotifications, nextRequests, nextConversations] = await Promise.all([profileHub.listNotifications(), profileHub.listPendingRequests(), profileHub.listConversations(session.user.id)]);
      setNotifications(nextNotifications); setRequests(nextRequests); setConversations(nextConversations);
      if (selectedId) { setMessages(await profileHub.listMessages(selectedId)); await profileHub.markConversationRead(selectedId, session.user.id); }
  ...
  const chooseConversation = async id => { setSelectedId(id); setTab("messages"); setBusy(true); try { setMessages(await profileHub.listMessages(id)); await profileHub.markConversationRead(id, session.user.id); } catch (chooseError) { setError(chooseError.message); } finally { setBusy(false); } };
  const sendMessage = async () => { if (!selectedId || !messageText.trim()) return; setBusy(true); try { await profileHub.sendMessage(selectedId, session.user.id, messageText); setMessageText(""); setMessages(await profileHub.listMessages(selectedId)); } catch (sendError) { setError(sendError.message); } finally { setBusy(false); } };
  const respond = async (request, decision) => { setBusy(true); try { await professionalGraph.respondToConnection(request.id, decision); await load(); } catch (respondError) { setError(respondError.message); } finally { setBusy(false); } };
  const openNotification = async notification => {
    if (!notification.read_at) await profileHub.markNotificationRead(notification.id);
    if (notification.notification_type === "MESSAGE" && notification.entity_id) { await chooseConversation(notification.entity_id); return; }
    if (notification.notification_type === "CONNECTION_REQUEST") { setTab("requests"); await load(); return; }
    if (["FOLLOW","SUBSCRIPTION","CONNECTION_ACCEPTED"].includes(notification.notification_type) && notification.actor_profile_id) { openProfile(notification.actor_profile_id); return; }
    if (notification.entity_type === "ARTICLE" && notification.entity_id) { window.location.hash = `article-${notification.entity_id}`; return; }
    await load();
  };
  const selectedConversation = conversations.find(item => item.id === selectedId);
  const unreadCount = notifications.filter(item => !item.read_at).length;
  return <div className="inbox-page"><header className="feed-topbar"><button className="feed-logo" onClick={openFeed}><img src={`${import.meta.env.BASE_URL}stackedin-icon.webp`} alt="StackedIN" /></button><label><Search size={17} /><input value="Notifications, requests, and messages" readOnly /></label><nav><button onClick={openFeed}><Home size={18} /><span>Home</span></button><button className="active"><Bell size={18} /><span>Inbox</span>{unreadCount > 0 && <em>{unreadCount}</em>}</button><button className="feed-avatar" onClick={() => openProfile(session.user.id)}><b>{context?.profile?.display_name?.charAt(0).toUpperCase() || "S"}</b><span>Me</span></button></nav></header><main className="inbox-shell"><aside className="inbox-nav"><header><span>Communication hub</span><h1>Inbox</h1></header><button className={tab === "notifications" ? "active" : ""} onClick={() => setTab("notifications")}><Bell size={17} />Notifications{unreadCount > 0 && <b>{unreadCount}</b>}</button><button className={tab === "requests" ? "active" : ""} onClick={() => setTab("requests")}><Users size={17} />Pending requests{requests.length > 0 && <b>{requests.length}</b>}</button><button className={tab === "messages" ? "active" : ""} onClick={() => setTab("messages")}><MessageCircle size={17} />Messages{conversations.filter(item => item.unread).length > 0 && <b>{conversations.filter(item => item.unread).length}</b>}</button><footer><ShieldCheck size={15} /><p>Messages are visible only to conversation members and protected by Supabase Row Level Security.</p></footer></aside><section className="inbox-content">{error && <div className="native-inline-error">{error}</div>}{tab === "notifications" && <><header className="inbox-heading"><div><span>Updates</span><h2>Notifications</h2></div>{unreadCount > 0 && <button onClick={() => profileHub.markAllNotificationsRead(session.user.id).then(load)}><Check size={14} />Mark all read</button>}</header><div className="notification-list">{notifications.map(notification => <button className={!notification.read_at ? "unread" : ""} key={notification.id} onClick={() => openNotification(notification)}><div>{notification.actor?.avatar_url ? <img src={notification.actor.avatar_url} alt="" /> : notification.notification_type === "MESSAGE" ? <MessageCircle size={17} /> : <Bell size={17} />}</div><section><strong>{notification.title}</strong><p>{notification.body}</p><span>{notification.actor?.display_name || "StackedIN"} · {formatDate(notification.created_at)}</span></section>{!notification.read_at && <i />}</button>)}{!notifications.length && <div className="inbox-empty"><Bell size={26} /><h3>You are all caught up</h3><p>Connection, publishing, and message activity will appear here.</p></div>}</div></>}{tab === "requests" && <><header className="inbox-heading"><div><span>Your network</span><h2>Pending connection requests</h2></div></header><div className="request-list">{requests.map(request => <article key={request.id}><div>{request.requester?.avatar_url ? <img src={request.requester.avatar_url} alt="" /> : String(request.requester?.display_name || "S").charAt(0)}</div><section><h3>{request.requester?.display_name || "StackedIN professional"}</h3><p>{request.requester?.headline || request.requester?.current_company || "Professional connection request"}</p><span>Requested {formatDate(request.requested_at)}</span></section><footer><button disabled={busy} onClick={() => respond(request,"DECLINED")}>Decline</button><button disabled={busy} onClick={() => respond(request,"ACCEPTED")}><Check size={14} />Accept</button></footer></article>)}{!requests.length && <div className="inbox-empty"><Users size={26} /><h3>No pending requests</h3><p>Your professional graph is currently tidy.</p></div>}</div></>}{tab === "messages" && <div className="messaging-layout"><aside><header><span>Conversations</span><h2>Messages</h2></header>{conversations.map(conversation => <button className={`${conversation.id === selectedId ? "active" : ""} ${conversation.unread ? "unread" : ""}`} key={conversation.id} onClick={() => chooseConversation(conversation.id)}><div>{conversation.otherProfile?.avatar_url ? <img src={conversation.otherProfile.avatar_url} alt="" /> : String(conversation.otherProfile?.display_name || "S").charAt(0)}</div><section><strong>{conversation.otherProfile?.display_name || conversation.title || "Conversation"}</strong><p>{conversation.lastMessage?.body || "Start the conversation"}</p><span>{formatDate(conversation.updatedAt)}</span></section></button>)}</aside><section className="message-thread">{selectedConversation ? <><header><div>{selectedConversation.otherProfile?.avatar_url ? <img src={selectedConversation.otherProfile.avatar_url} alt="" /> : String(selectedConversation.otherProfile?.display_name || "S").charAt(0)}</div><section><strong>{selectedConversation.otherProfile?.display_name || "StackedIN professional"}</strong><span>{selectedConversation.otherProfile?.headline || "Professional connection"}</span></section><button onClick={() => openProfile(selectedConversation.otherProfile?.id)}><UserRound size={15} />Profile</button></header><div className="message-list">{messages.map(item => <article className={item.sender_profile_id === session.user.id ? "mine" : "theirs"} key={item.id}><div>{item.deleted_at ? "Message removed" : item.body}</div><span>{formatDate(item.created_at)}{item.edited_at ? " · edited" : ""}</span></article>)}</div><form onSubmit={event => { event.preventDefault(); sendMessage(); }}><textarea value={messageText} onChange={event => setMessageText(event.target.value)} onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); sendMessage(); } }} placeholder="Write a message… Enter to send · Shift+Enter for a new line" maxLength={8000} /><button disabled={busy || !messageText.trim()}><Send size={16} /></button></form></> : <div className="inbox-empty"><MessageCircle size={28} /><h3>Select a conversation</h3><p>Messaging becomes available after a connection is accepted.</p></div>}</section></div>}</section></main></div>;
}
...
export default function App() {
  ...
  const openProfile = profileId => navigate(profileId && profileId !== session?.user?.id ? `profile-${profileId}` : "profile");
  ...
  if ((route === "profile" || route.startsWith("profile-")) && session) return <ProfileExperience session={session} targetProfileId={route.startsWith("profile-") ? route.slice(8) : session.user.id} openFeed={() => navigate("feed")} openWrite={() => navigate("write")} openInbox={openInbox} signOut={signOut} />;
  if (route === "inbox" && session) return <InboxExperience session={session} initialConversationId={sessionStorage.getItem("stackedin-inbox-conversation") || ""} openFeed={() => navigate("feed")} openProfile={openProfile} />;
